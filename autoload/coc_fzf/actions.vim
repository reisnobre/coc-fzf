...
function! coc_fzf#actions#fzf_run(...) abort
  ...
  let l:diags = CocAction('actionsList')
  if !empty(l:diags)
    let expect_keys = join(keys(get(g:, 'fzf_action', s:default_action)), ',')
    let l:opts = {
          \ 'source': s:get_actions(l:diags, l:current_buffer_only),
